...
2. **每次改动完成后，必须编写或更新相关测试，并在交付给用户前，确保所有测试和验证全部通过。**
   （测试基建在 v1.7.0 建立：vitest + jsdom，`npm test`；建立后所有改动均需遵循。）

## 当前项目计划

**见 [docs/ROADMAP.md](docs/ROADMAP.md)** —— 2026-09 用户实测 10 项问题的完整迭代计划
（v1.7.0 团员侧修复/QQ 机器人/测试基建 → v1.8.0 订单改版 → v1.9.0 国际计算/活动管理 → v2.0.0 新工具/导出/响应式），
含「P0 团员侧线上故障分析」与线上排查清单。开展任何任务前先读它。
