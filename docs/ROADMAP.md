...
| # | 用户问题 | 对应版本 | 状态 |
|---|----------|----------|------|
| 0 | 团员侧线上无法跑通（排障 + 修复，含问题 1） | v1.7.0 | ✅ 代码修复完成，待线上重跑 schema 验证 |
| 1 | 团员提交的信息（QQ 绑定、地址等）无法保存 | v1.7.0 | ✅ 代码修复完成，待线上重跑 schema 验证 |
| 2 | QQ 机器人 @ 与私聊完全不可用 | v1.7.0 | ✅ 代码修复完成（私聊链路补全 + relay 节流 + https 校验），待实测 |
| 3 | 外币价格：录入取消自动转换 → 订单管理批量生成；保留外币原价可查 | v1.8.0 | 未开始 |
| 3-附 | 录入页非人民币两种勾选模式；制品类型面板内快捷新建 | v1.8.0 | 未开始 |
| 10 | 录入页购买者多行换行 = 同款订单批量导入 | v1.8.0 | 未开始 |
| 6 | 订单管理缺编辑/备注；表格换行折叠、贴边 | v1.8.0 | 未开始 |
| 5 | 加权国际单价与均价合并、缺目标金额差值 → 恢复 + 悬浮仪表盘 | v1.9.0 | 未开始 |
| 7 | 活动管理缺购买人/备注/快递单号/模糊出荷日期 | v1.9.0 | 未开始 |
| 4 | 全站响应式 UI 修订（表格间距、手机/电脑适配） | v1.9.0 + v2.0.0 | 未开始 |
| 8 | 新工具：限购产品账号购买计划计算器 | v2.0.0 | 未开始 |
| 9 | 表格图片导出 + 表格文件下载铺开到主要场景 | v2.0.0 | 未开始 |
