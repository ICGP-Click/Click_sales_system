...
> 更新日期：2026-09-06 · 版本 **v3.0.0**（v3：管理员账号体系重设计——部署时初始化超管 + 应用内管理员管理，脱离 Supabase Auth；详见 docs/PLAN-AUTH-REDESIGN.md）（含 2026-09-06 十项问题迭代：v1.7.0 团员侧修复 + QQ 机器人 + 测试基建 → v1.8.0 订单改版 → v1.9.0 国际计算/活动管理 → v2.0.0 限购计算器/导出/响应式；变更明细见 CHANGELOG.md，任务溯源见 ROADMAP.md）

## 当前状态速览（2026-09-06 迭代后）

- **代码层**：十项实测问题全部修复/实现，vitest 60 用例全绿（`npm test`）。
- **⚠️ 待线上操作（用户执行）**：
...
- **QQ 机器人**：私聊推送链路（逐人 `send_private_msg` + relay 1s 节流）、群发 @ 映射修正、https 强制校验。
