        ...
        window.renderBgConfig = function() {
            ...
            <div class="flex flex-col gap-3">
                <div><label class="text-xs text-gray-500">背景类型</label>
                ...
                <div id="bgColorRow" style="display:${bgType==='color'?'block':'none'}">
                    <label class="text-xs text-gray-500">背景颜色</label>
                ...
                <div id="bgImageRow" style="display:${bgType==='image'?'block':'none'}">
                    <label class="text-xs text-gray-500">背景图片（自动添加遮罩）</label>
                    <div class="flex gap-2 items-center">
                    ...
                    <div class="mt-2 flex items-center gap-2">
                        <label class="text-xs text-gray-500">遮罩透明度:</label>
                        <input type="range" id="bgOverlaySlider" min="0" max="60" value="${Math.round(bgOpacity*100)}" class="flex-1">
                        <span id="bgOverlayLabel" class="text-xs text-gray-500 w-8">${Math.round(bgOpacity*100)}%</span>
                    </div>
                    <div id="bgPreviewArea" class="mt-2 ${bgUrl?'':'hidden'}">
                        <p class="text-xs text-gray-400 mb-1">预览（含遮罩效果）:</p>
            ...
            <div class="space-y-3">
                <div class="flex items-center justify-between py-2 border-b border-gray-100">
                    <div>
                        ...
                        <p class="text-xs text-gray-400">首页"吃谷成就排名"入口</p>
                    </div>
        ...
        window.applyFeatureToggles = function() {
            const piggyBtn = document.querySelector('#portal-screen button[onclick*="rank-screen"]');
            if (piggyBtn) {
                piggyBtn.style.display = isFeatureEnabled('piggyRank') ? '' : 'none';
            }
        };
