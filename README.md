...
| 信息录入 | Excel 导入（排谷/拼谷/闲鱼矩阵自动识别）；手动录入支持**外币双模式**（直接输入外币价 / 计算器计算）、**多行购买者批量导入**、类型面板内**快捷新建** |
...
| 通知公告 | 催缴/发货通知自动生成；QQ 机器人推送（**群发@ / 逐人私聊 / 全部** 三模式） |
...
3. 通知页推送三选：「群发(@)」@已绑定 QQ 的成员；「私聊」逐人发送（relay 自动 1s 节流防风控）；「全部」先私聊后群发兜底。
